...
if __name__ == "__main__":
    ...
    for sid in range(5):
        ...
        X_train, X_val, _, _ = train_test_split(X, y,stratify=y, test_size=0.1,random_state=1)
    ...
    for i, (valitem, trainitem) in enumerate(zip(val,train)):
        with open("cifar100_train_blurry10_rand1_cls20_task%d.json"%(i),"w") as f:
            json.dump(trainitem,f)
        with open("cifar100_val_blurry10_rand1_cls20_task%d.json"%(i),"w") as f:
            json.dump(valitem,f)
